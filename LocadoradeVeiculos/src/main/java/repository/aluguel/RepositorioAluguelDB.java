...
public class RepositorioAluguelDB implements RepositorioAluguel, Serializable {
    ...
    public List<Aluguel> getAll(Cliente cliente) {
        ...
        String sql = "SELECT * FROM ALUGUEL WHERE CLIENTE = ?";
        try (PreparedStatement ps = Conexao.getConexao().prepareStatement(sql)) {
    ...
    public void atualizarStatusAluguel(String placa) {
    String sql = "UPDATE VEICULO SET ALUGADO = ? WHERE PLACA = ?";
    try (PreparedStatement ps = Conexao.getConexao().prepareStatement(sql)) {
        boolean alugado = ps.getBoolean("ALUGADO");
